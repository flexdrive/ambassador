...
ambassador.docker: Dockerfile base-go.docker base-py.docker $(ENVOY_FILE) $(WATT) $(KUBESTATUS) $(WRITE_IFCHANGED) python/ambassador/VERSION.py FORCE
...
build/kat/client: kat-client.docker
.PHONY: build/kat/client
...
build/kat/server: kat-server.docker
.PHONY: build/kat/server
